...
export default function PetsItem({ pet }: { pet: Pet }) {
  ...
  return (
    <li>
      <button
        onClick={onSelectPet}
        className={cn("flex items-center p-4 gap-3 w-full cursor-pointer h-[70px] text-base hover:bg-light focus:bg-light transition", {'bg-light': pet.id === selectedPet?.id})}
      >
